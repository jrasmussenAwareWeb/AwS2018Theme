...
Version: 1.4
NOTE: If updating this plugin, be sure to reinclude $('div.cardStack').awsCardStack(); before ending jQuery line at the bottoom of this file
**/
    ...
    $.fn.awsCardStack = function(options){
        ...
        CardStack.prototype = {
            settings:{
                ...
                swipeThreshold: 60 // number of pixels someone has to swipe before actually triggering the swipe event
            },
            ...
            init: function(){
                ...
                oContainer.on('touchstart', function(e){
                    oContainer.data('touchstart', e.originalEvent.changedTouches[0].screenX);
                }).on('touchend', function(e){
                    var iStart = oContainer.data('touchstart'),
                        iEnd = e.originalEvent.changedTouches[0].screenX,
                        iDiff = iStart - iEnd,
                        oHeadings = oContainer.find(_Config.headings),
                        oActive = oHeadings.find('li.' + _Config.activeClass).first(),
                        oFirst = oHeadings.children('ul').find('li:first-child'),
                        oLast = oHeadings.children('ul').find('li:last-child');

                    // Don't continue if user didn't swipe far enough
                    if(Math.abs(iDiff) < _Config.swipeThreshold){return;}

                    // Determine if swiping left(prev) or right(next)
                    if(iDiff > 0){ // next
                        if(oLast.hasClass(_Config.activeClass)){
                            oFirst.find('a').trigger('mouseover');
                        }else{
                            oActive.next().find('a').trigger('mouseover');
                        }
                    }else{ // previous
                        if(oFirst.hasClass(_Config.activeClass)){
                            oLast.find('a').trigger('mouseover');
                        }else{
                            oActive.prev().find('a').trigger('mouseover');
                        }
                    }
                });
        ...
        // Standard jQuery return for collections of elements triggering a plugin
